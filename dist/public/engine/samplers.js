...
const GridPosition = (x, y, z, size, step) => {
    return {
        x: -size / 2 + (x + 0.5) * step,
        y: -size / 2 + (y + 0.5) * step,
        z: -size / 2 + (z + 0.5) * step,
...
export const LatLongSampler = {
    ...
    generate(boundary, options) {
        const { thetaSegments = this.defaults.thetaSegments, phiSegments = this.defaults.phiSegments, displayRadius = this.defaults.displayRadius } = options;
...
export const FibonacciSampler = {
    ...
    generate(boundary, options) {
        const { sampleCount = this.defaults.sampleCount, displayRadius = this.defaults.displayRadius } = options;
...
export const UniformVoxelSampler = {
    ...
    generate(decoder, options) {
        const { size = this.defaults.size, resolution = this.defaults.resolution } = options;
...
export const UniformPointCloudSampler = {
    ...
    generate(decoder, options) {
        const { size = this.defaults.size, resolution = this.defaults.resolution, threshold = this.defaults.threshold } = options;
...
export const SurfacePointCloudSampler = {
    ...
    generate(decoder, options) {
        const { size = this.defaults.size, resolution = this.defaults.resolution, threshold = this.defaults.threshold, isoValue = this.defaults.isoValue } = options;
        const dataset = new PointDataset(decoder.metadata);
        dataset.metadata.sampler = { ...this.metadata };
        const step = size / resolution;
        for (let z = 0; z < resolution; z++) {
