...
export const viewController = () => {
    ...
    if (!boundaryViewer)
        throw new Error("Missing #boundary-viewer element");
    const projectionViewer = document.getElementById("projection-viewer");
    if (!projectionViewer)
        throw new Error("Missing #projection-viewer element");
    const viewerSize = Math.min(boundaryViewer.clientHeight, projectionViewer.clientHeight);
    ...
    // Camera controls
    const boundaryControls = new OrbitControls(camera, boundaryRenderer.domElement);
    const projectionControls = new OrbitControls(camera, projectionRenderer.domElement);
    return {
        camera,
        boundaryViewer,
        projectionViewer,
...
const SceneController = (scene) => {
    ...
    let autoRotate = true;
    const setupGroup = () => {
        if (scene.getObjectByName(groupName))
            return;
    ...
    const setupLights = () => {
        ...
        if (scene.getObjectByName(mainLightName))
            return;
