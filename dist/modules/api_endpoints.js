...
// - ~ - ~ - ~ - ~ - ~ - ~ - ~ -
// - ~ - Helper Functions - ~ -
const RegisterBoundaryEndpoint = (apiRouter, path, operation) => {
    apiRouter.get(path, (req, res) => {
        ...
        if (!Number.isFinite(position.x) ||
            ...
            SendError(res, 400, APIErrorCodes.INV_PARAM, "x, y and z must be finite numbers", swaggerURL.href);
            return;
        }
        res.json({ value: boundaryField.sample(position) });
    });
};
const RegisterDecoderEndpoint = (apiRouter, path, operation) => {
    apiRouter.get(path, (req, res) => {
        // The first OpenAPI tag determines the Swagger UI navigation category
        const tag = operation.tags[0]; // Ex. Decoders
        const operID = operation.operationId; // Ex. radialDecoder - the actual callable function name
        ...
        if (!IsBFieldValid(fieldName)) {
            SendError(res, 400, APIErrorCodes.INV_PARAM, `Invalid boundary field: ${fieldName}`, swaggerURL.href);
            return;
        }
...
export const CreateAPIEndpoints = (apiRouter, openAPISpec) => {
    let projectSchema;
    try {
