...
import { HandleRenderOptions, PopulateSelect, PopulateSamplerSelect, 
         RenderableListController, AddRenderable, SetPageLayout 
        } from "./adapter/utility.js";
...
metadataElement.textContent = JSON.stringify(
    renderables.boundary.pipelineDataset.metadata, null, 2);
...
renderButton.addEventListener("click", () => {
    ...
    switch (samplerType) {
        ...
        default:
            ...
            if (isProjectionCached) return;

            if (pipelineState.meshGen) samplerType = "mesh";
...
boundaryRenderList.addEventListener("dblclick", () => {
    ...
    if (renderableKey) boundaryController.recover(boundarySceneController, renderables, renderableKey);
});
...
projectionRenderList.addEventListener("dblclick", () => {
    ...
    if (renderableKey) projectionController.recover(projectionSceneController, renderables, renderableKey);
});

boundaryRenderList.addEventListener("keydown", event => {

    const renderableKey = boundaryRenderList.value;
    if (!renderableKey) return;
