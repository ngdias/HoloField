...
import { GenerateBoundaryDataset, GeneratePointCloudDataset, 
         GenerateVoxelDataset, GenerateMeshDataset } from "../engine/pipeline.js";
...
export class VoxelProjectionRenderable {
    ...
    static prepare(sceneController, renderable, boundaryName, decoderName, samplerName) {
        ...
        renderable.sceneObject = new THREE.InstancedMesh(
            cube,
            renderable.material,
            occupiedVoxels
        );
        ...
        for (let i = 0; i < dataset.values.length; i++) {
            ...
            */
            ...
            renderable.color.setRGB(
                0,
                0.2 + 0.8 * (1 - normalizedValue),
                0.3 + 0.7 * (1 - normalizedValue)
            ); // Cyan?
        ...
        console.assert(instance === occupiedVoxels,
            `Voxel count mismatch: created ${instance}, expected ${occupiedVoxels}`
        );
...
export class CubeDemoRenderable {
    ...
    static prepare(sceneController, renderable) {
        ...
        renderable.sceneObject = new THREE.Mesh(
            geometry,
            material
        );
